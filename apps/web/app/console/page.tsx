...
import { ScrollMask } from "@/components/scroll-mask";
...
export default async function FleetPage() {
  ...
  return (
    <VStack as="main" gap={8} width="100%" className="min-w-0">
      {/*
        The reveal opens onto the project's own Open Graph card, not a stock
        photograph. `/opengraph-image` is generated by this app from the same
        mark the favicon uses, so the console pulls no third-party image and
        needs no `next.config` remote allowance — which also keeps the "nothing
        here comes from somewhere you cannot check" claim intact on the one
        screen that makes it loudest.

        `radius` matches the frames below it and `background` stays transparent
        so the console surface shows through rather than a second black.
      */}
      <ScrollMask
        variant="curtain"
        word="NYMSPACE"
        alt="Nymspace — ENS as the identity layer"
        feather={16}
        radius={12}
        overlay={0.35}
        className="-mt-2"
      >
        <VStack gap={2} align="center">
          ...
          <Text type="supporting">
            {fleet.agents.length} agents on ENSv2 Sepolia
          </Text>
        </VStack>
      </ScrollMask>
