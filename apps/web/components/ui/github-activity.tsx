...
const ContributionGrid = ({
  ...
  label: string;
  reduceMotion: boolean | null;
  onDaySelect?: (day: Contribution) => void;
          ...
          <div key={weekIndex} className="flex flex-col" style={{ gap }}>
            {week.map((day) => (
              <motion.div
                ...
                tabIndex={onDaySelect && day.count > 0 ? 0 : undefined}
                onKeyDown={
                  onDaySelect
                    ? (event) => {
                ...
                className={cn(
                  "shrink-0 rounded-[3px] bg-foreground/[0.08] outline-none",
...
export type GitHubActivityProps = React.ComponentProps<"div"> & {
  ...
  /** Fired when a day cell is clicked. Added for repo-level commit drill-down. */
  onDaySelect?: (day: Contribution) => void;
  /** ISO date (YYYY-MM-DD) of the currently selected day, for the ring highlight. */
  selectedDate?: string | null;
};
