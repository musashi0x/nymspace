...
/**
 * One seeded PRNG, rolled once per sentence at init.
 *
 * Everything random in the loop — dash jitter, letter drift offsets, fleck
 * headings, the burst kick — is drawn here and then frozen. A loop that rolled
 * dice per frame would play differently every cycle, which reads as noise
 * rather than as choreography, and would make a visual regression impossible
 * to see.
 */
...
export interface LoudBurstOptions {
  /** A concrete family name. A `var(--…)` here is an invalid canvas font. */
  font?: string;
  skin?: Skin;
  /** Which sentence to open on. */
  sentence?: number;
  /** False plays exactly one cycle — the preloader's mode. */
  loop?: boolean;
  /** Fired the moment the cycle wraps, after the two blank ticks. */
  onCycle?: () => void;
}
...
export class LoudBurst {
  ...
  private onCycle: (() => void) | undefined;
  ...
  constructor(
    ...
    options: LoudBurstOptions = {},
  ) {
    this.ctx = canvas.getContext("2d");
    this.ok = !!this.ctx;
    this.font = options.font ?? "sans-serif";
    this.paper = options.skin?.paper ?? PAPER;
    this.inkRgb = hexRgb(options.skin?.ink ?? INK);
    this.loop = options.loop ?? true;
    this.onCycle = options.onCycle;
    this.applySentence((options.sentence ?? 0) % SENTENCES.length);
  ...
  private applySentence(i: number) {
    ...
    this.bodyRgb = mixArr(this.inkRgb, hexRgb(th.body), BODY_TINT);
    ...
    /**
     * The hue slide, mapped onto this sentence's palette.
     *
     * `strokes.ts` carries absolute hues because the flight was drawn, not
     * generated. Each group keeps the *journey* those hues describe — where it
     * starts, where it ends, how far it travels — but the two endpoints are
     * snapped to the nearest inks in the current palette, so a group that
     * crossed purple into blue still crosses in the same direction after the
     * sentence changes.
     */
  ...
  resize() {
    ...
    /**
     * Repaint immediately, at whatever tick is current.
     *
     * Assigning `canvas.width` clears the bitmap, so a resize mid-cycle leaves
     * the frame blank until the next tick fires — and if the loop is parked,
     * because the tab is hidden or the window is being dragged, "the next tick"
     * can be seconds away. The visible symptom is the last frame stranded in a
     * corner of a bigger canvas.
     */
    const current = this.lastTick;
    this.lastTick = -1;
    if (current >= 0) this.render(current);
    else if (!this.running) this.renderStill();
  }

  /**
   * The stage, not the canvas.
   *
   * Every measurement in `params.ts` and every keyframe in `strokes.ts` is a
   * fraction of a 700x392 frame. Scaling those against the raw viewport would
   * make the composition a function of the window's aspect ratio: on a tall
   * phone the type would be set at a sixth of the screen height and the
   * sentence would run off both edges. So a 700x392 stage is fitted inside the
   * viewport, the type and the strokes are scaled to *it*, and the strokes
   * still leave through the real edges — which is what the finale is for.
   */
  private buildLayout() {
    const W = this.canvas.width;
    const H = this.canvas.height;
    this.stageH = Math.min(H, W / (REF_W / REF_H));
    this.refS = this.stageH / REF_H;
    this.fontPx = FONT_FRAC * this.stageH;
    this.baseline = (H - this.stageH) / 2 + BASELINE_FRAC * this.stageH;
  ...
  private layoutSentence() {
    ...
    let lineW = this.measureLine();

    /**
     * A five-word sentence at 12.2% of the stage height fits the reference
     * frame, and does not fit every word in every sentence at every aspect —
     * "good design shouts then leaves" is a third longer than "break the grid
     * then go". Advances scale linearly with the font size, so one corrective
     * pass is exact rather than iterative.
     */
    const maxW = LINE_FIT * W;
    if (lineW > maxW) {
      this.fontPx *= maxW / lineW;
      this.gap = GAP_EM * this.fontPx;
      lineW = this.measureLine();
    }
    ...
    /**
     * The glide, read off the table rather than followed.
     *
     * The line starts centred on its first word alone and travels to the
     * layout position it will hold for the rest of the cycle. `RECENTER` is
     * that trajectory measured tick by tick, including the overshoot; an
     * exponential follower cannot reproduce it, because it cannot pass its
  ...
  /**
   * A tapered dart: thin tail, full-width belly, thin head, with a bend.
   *
   * Drawn as a filled outline rather than a stroked line, because a stroke has
   * one width for its whole length and these need three. The width profile is
   * `sin(pi * u^skew) ^ belly`, which is what gives the dash its brush look,
   * and the gradient runs tail-to-head so the hue slide is visible *along* each
   * dash rather than only between one dash and the next.
   */
  private drawDart(
    tailX: number,
    tailY: number,
  ...
  /**
   * Blur as a downscale/upscale chain.
   *
   * Halve one to three times with bilinear filtering, then double back up. One
   * big jump down and back reads as mosaic — visible square blocks — because
   * the filter only ever samples a 2x2 neighbourhood. Repeated halving is what
   * widens the effective kernel, which is what makes it read as defocus.
   */
  private blurDraw(
  ...
  /**
   * The word's last jittered frame, sliced into ribbons.
   *
   * Rasterised once and scanned row by row for runs of ink, so the pieces that
   * fly are the shapes of the letters rather than rectangles over them. Each
   * run is then handed to the hatch group whose flight direction best matches
   * where the run sits relative to the word's centre — the burst therefore
   * carries the word outward along the same paths the drawn strokes take,
   * which is what makes the two halves of the effect read as one event.
   */
  private captureSegs() {
    this.segsReady = true;
    this.segs = [];
  ...
  private renderBurstWords(t: number) {
    ...
    if (!this.segs.length) {
      /* No pixels to slice — a tainted or unreadable buffer. The word still
         has to leave, so it leaves whole rather than not at all. */
      if (alpha > 0) {
        ctx.save();
        ctx.globalAlpha = alpha;
  ...
  /**
   * The rAF loop redraws only when the 30-ish-fps tick changes.
   *
   * rAF fires at the display's rate — 120Hz on a ProMotion panel — and the
   * animation has 51 states. Redrawing every callback would do four times the
  ...
  /** The one frame reduced motion gets: mid-burst, everything legible. */
  renderStill() {
    if (!this.ok) return;
