/**
 * The hatch groups and the finale strokes, as measured keyframe paths.
 *
 * Times are in *reference* ticks — the frame numbering the paths were drawn
 * against — and the engine shifts them onto its own clock with `STROKE_OFFSET`.
 * Each key carries the group's hue, so a group's colour is interpolated along
 * its flight: purple crosses into blue, orange warms into gold. That hue slide
 * is the signature of the burst and the reason these are keyframes rather than
 * a particle system, which would give every dash the same colour it started
 * with.
 */
...
/** Linear interpolation between the two keys bracketing `t`. */
