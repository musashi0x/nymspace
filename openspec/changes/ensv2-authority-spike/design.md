...
- **Agent controller key custody.** `docs/` never decides this. `docs/19_ENV_AND_CONFIG.md` lists Privy keys, RPC, and contract addresses, but no agent controller key variable. Two options with different demo consequences: a server-held key (scriptable, survives restart, matches "the agent acts on its own") or a second browser wallet (visually convincing on stage, fragile, requires a second account mid-demo). The spike can proceed with a server-held key, but `docs/15_DEMO_SCRIPT.md` should be reconciled before Day 4.
- **Which parent label, and bought or migrated.** `docs/17` Risk 2 says branding must not depend on a particular parent name and to use a dedicated hackathon parent if needed. Still undecided, and it gates U1, but the options are now concrete: purchase through `ETHRegistrar` with commit-reveal and an ERC 20, or migrate an ENSv1 Sepolia name already held through the migration controllers in the same deployment. Migration is faster if such a name exists, which is a question only the team can answer.
