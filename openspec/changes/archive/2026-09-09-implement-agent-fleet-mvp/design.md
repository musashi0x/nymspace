...
**D12: Postgres for the store, Gemini for the ranking step.**
Task 1.6 forced both, and both add exactly one variable — `DATABASE_URL` and `GEMINI_API_KEY` — to `.env.example` and `turbo.json` in the same edit. Postgres over a file because the activity route in task 6.10 filters by agent, source, type, and status, and four filter dimensions over an append-only log is a query rather than an array scan; the restart test in 2.6 is then a property of the engine instead of something the code has to be careful about. The cost is real and accepted: the demo depends on a running service, and section 8's rehearsal has to start it. Gemini's model id is pinned in code rather than in the environment, because a variable that selects a model lets a deploy change what the explanation validator in 4.10 is validating against. *Alternative considered:* a deterministic ranker with no provider, which would cut a dependency and a whole class of hallucination risk. Rejected because the proposal claims an AI track, and D4's separation — the model gets a tool, not a transcript — is the part worth demonstrating.

**D13: The ERC 8004 registration file is a base64 data URI, and the ENS claim is a `services` entry inside it.**
Found while implementing task 3.8, and it changes what that task is. The registration is `register(agentURI)` on an ERC 721; the ENS claim is not an on-chain field at all. Agent0's `identity-registry.ts` parses the registration file from exactly two `agentURI` forms — `ipfs://` and `data:application/json;base64,` — and an `https://` URI is stored with `agentURIType: "https"` and then never fetched, so it produces no `registrationFile`, no `ens`, and no `mcpEndpoint`. Discovery would return the agent as an untyped shell. The data URI is therefore the only form that needs no new dependency: the whole file rides in calldata, there is no pinning service, no availability window, and no fourth credential on the critical path. The claim itself is `services: [{ name: "ens", endpoint: "research.nymspace.eth" }]`, which is what populates `AgentRegistrationFile.ens` and what task 3.9 must assert against. *Alternative considered:* IPFS via a pinning service, which is what the ERC's own examples show. Rejected for this change — it adds a sign-up to the critical path for a file of a few hundred bytes, and `docs/17` Risk 3 is already about depending on infrastructure we do not run. The cost accepted is that updating the registration file is a transaction rather than a re-pin, which for three demo agents is not a cost.

**D14: The ERC 8004 registration goes on Base Sepolia; the names stay on Sepolia.**
Forced by an outage rather than chosen. Ethereum Sepolia's Agent0 subgraph has a single indexer stuck at block 10476469 with `hasIndexingErrors`, and the gateway answers every entity query with `bad indexers: BadResponse(no attestation: indexing_error)` — `_meta` still answers, which is why Gate 0 passes while Gate B cannot run at all. Base Sepolia is healthy and returns agents. ENSv2 exists only on Sepolia, so the names cannot move; the ERC 8004 identity registry is deployed at the same address on both networks, so the registration can. The binding therefore spans two chains, and the only mechanical difference is the ENSIP 25 key's chain reference — `0x014a34` instead of `0xaa36a7` — which `agentRegistrationKey` already handles and `keys.test.ts` already asserts. *Alternative considered:* register on Sepolia anyway and wait for the indexer. Rejected because D8's whole argument for registering early is to convert indexing latency into a wait that runs in parallel, and a stuck indexer is not a wait with a known end. The cost accepted is that ADR 009 said not to claim the cross-chain track, and this does not claim it: it is one registration on the network whose indexer works, not a cross-chain query surface. If Sepolia's indexer recovers, registering there as well is one script run.
...
- ~~**Which LLM provider, and does the discovery step run on the API or in a Next route?**~~ Settled at task 1.6, recorded as D12: Gemini, on the API, behind the `server-only` guard with `GRAPH_API_KEY`. `GEMINI_API_KEY` is declared in both files; the model id is pinned in code.
- ~~**Persistence: SQLite file, Postgres, or JSON on disk?**~~ Settled at task 1.6, recorded as D12: Postgres, one `DATABASE_URL`, declared in both files.
